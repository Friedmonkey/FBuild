...
public partial class FriedAssembler : AnalizerBase<char>
{
    ...
    public byte[] Parse(string input, bool hasSymbols = true)
    {
        ...
        ParseInstructions(input);//, out int address_offset);
        this.Position = 0;
    ...
    private string FinalGenerator(bool includeSymbols, bool compact, byte version)
    {
        ...
        {
            if (declare.type.name == "interpolated_string")
            {
                string subStrings = "";
                foreach (char bite in declare.value)
                {
                    subStrings += (char)bite;
                }
                ...
                foreach (var subStringName in subStringNames)
                {
                    ...
                    if (idx == -1)
                    {
                        throw new Exception($"Varible \"{subStringName}\" not found! for interpolated string: {declare.name}");
                    }
                    bytes.AddRange(idx.VLQ());
                }
                AppendBytes(bytes.Count().VLQ());
                AppendBytes(bytes.ToArray());
            }
            else
            {
...
#warning todo optimize this so when it only skips 1 its just _ and not _1 saving 1 byte in the end
    ...
    public Declare ParseVarible(Type type = null, string name = null)
    {
        ...
        //    bytes.Add(0x00);
        //}
        else if ((Current == '$' && Peek(1) == '"') || Current == '%') //interpolated_string
        { 
            TypeCheck("interpolated_string");
            string str = HandleInterpolatedString();
            foreach (byte b in str) bytes.Add(b); //will need to be converted to string again to parse the names
        }
        ...
        else if (Current == '0' && Peek(1) is 'x' or 'X' && Peek(2).IsHexDigit()) //byte 0xFF (only parse 2 hex to make up the byte)
        {
        ...
        else if (Current == '0' && Peek(1) is 'b' or 'B' && char.IsDigit(Peek(2))) //binary 0b00001111 (only parse 8 (0 or 1) to make up the byte)
        {
            throw new NotImplementedException();
            TypeCheck("byte");
            Consume('0');
        ...
        string generatedName = name ?? $"_{autoGeneratedDeclares++}";
        return new Declare(type, generatedName, bytes.ToArray());

        bool IsTypeCompatible(string checkTypeName, string typeName)
        {
        ...
        bool InternalTypeCheck(string checkTypeName)
        {
            Type checkType = FindType(checkTypeName); //also checks if its valid type
            if (hasType)
            {
        ...
        void TypeCheck(string checkTypeName)
        {
            if (!InternalTypeCheck(checkTypeName))
                throw new Exception($"Type mismatch, expected \"{type.name}\" but got \"{checkTypeName}\" instead!");
        }
    ...
    public string ParseArgument() 
    {
        ...
        if (newVar.type.name == "interpolated_string") 
        {   //interpolated string in args arent const
            //interpolated strings when const get interpolated once
            //if not const itll get re interpolated every time it comes across it
            //so this one musnt be const or atleas i dont expect it to
    ...
    public void CheckName(string name)
    {
        ...
        if (TryGetType(name, out _)) throw makeException(nameof(Types));
    }
    ...
    public void ParseInstructions(string input)
    {
        ...
        while (Safe)
        {
            ...
            if (Current == ':') //label addresses
            {
                ...
                var label = GetDeclare(labelName);
                if (label is null)
                    throw new Exception($"label {labelName} not found!");

                if (label.value is not null)
                    throw new Exception($"label {labelName} was redefined!");

                label.value = Instructions.Count().VLQ();
                //var decl = new Declare(FindType("label"), labelName, Instructions.Count().VLQ());
                //AddDeclare(decl);
    ...
    public void GetLabels(string input)
    {
        ...
        while (Safe)
        {
            if (FindStart(":"))
            {
                ...
                CheckName(labelName);
                var decl = new Declare(FindType("label"), labelName) { isConst = true };
                AddDeclare(decl);
    ...
    public string ParseDeclares(string input)
    {
        ...
        while (Safe)
        {
            ...
            {
                throw new NotImplementedException("declare_struct");
...
#warning this is terrible, if we dont use it why set it to the max size, this will need to be done later
    ...
    private bool TryConsumeType(out Type type)
    {
        ...
        bool success = TryGetType(typename, out type);
        if (!success) Position -= typename.Length;
        return success;
    }
    ...
    private string HandleInterpolatedString()
    {
        var subStrings = ConsumeInterpolatedString();

        var subStringNames = new List<string>();
        foreach (var subString in subStrings)
        {
            if (subString.StartsWith("*STR_")) //its a string literal
            {
                var str = subString.Substring(5, subString.Length-5);
                List<byte> bytes = new();
                bytes.AddRange(str.Length.VLQ());
                foreach (byte b in str) bytes.Add(b);


                var strType = FindType("string");

                var existingVar = Declares.FirstOrDefault(existing =>
                    existing.isConst && //only makes sense if we can reuse constants because we know they wont mutate
                    strType.name == existing.type.name && //obviously needs the same type
                    bytes.Count() == existing.value.Length &&
                    Enumerable.SequenceEqual(bytes, existing.value) //also needs same value
                );
                ...
                if (existingVar is not null)
                {
                    ...
                    subStringNames.Add(existingVar.name);
                    //return existingVar.name;
                }
                else
                { 
                    string generatedName = $"_{autoGeneratedDeclares++}";
                    var decl = new Declare(FindType("string"), generatedName, bytes.ToArray()) { isConst = true, used = true };
                    subStringNames.Add(AddDeclare(decl));
                }
            }
            else
            {
                var decl = GetDeclare(subString);
                if (decl == null)
                    throw new Exception("Unable to get declare found in interpolated string:"+subString);
                decl.used = true; //used in this interpolated string
                subStringNames.Add(decl.name);
            }
        }

        return string.Join('»', subStringNames);
    }
    private List<string> ConsumeInterpolatedString()
    {
        var subStrings = new List<string>();
        if (Current == '%') //split string
        {   // %|"hello|world|how|are|you" //split by |

            // $"hello{} {name} how are you"
            Consume('%');
            char splitChar = Current;
            Position++;
            var splitString = ConsumeString();
            foreach (var str in splitString.Split(splitChar))
            {
                subStrings.Add("*STR_" + str);
            }
            return subStrings;
        }
        Consume('$');
        ...
        while (Safe && Current != '"')
        {
            if (Current == '{')
            {
                ...
                {   //$"hello{" "}world{" "}how{" "}are{" "}you" //does look somewhat cleaner if u wanna split by space
                    Consume('"');
                    consumed = ConsumeUntil('"');
                    Consume('"');
                    subStrings.Add("*STR_" + consumed);
                }
                else
