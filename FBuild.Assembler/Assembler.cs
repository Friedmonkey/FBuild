...
public partial class FriedAssembler : AnalizerBase<char>
{
    ...
    public byte[] Parse(string input)
    {
        ...
        ParseVars(input); //collect all dynamic varible names, so we can turn them intro addreses
        this.Position = 0;
    ...
    private string FinalGenerator(string instructions, UInt64 instructions_offset, Size meta, bool includeSymbols, byte version)
    {
        ...
        {
            //if (dec.value.Count() == 0)
            //    sb.Append(((UInt64)0).ToFixedByteArray(meta_size));
            //else
...
#warning should this be address or no?
        ...
        if (autoDeclare)
        {
            ...
            int index = AddDeclare(new Declare(generatedName, bytes.ToArray()));
            bytes.Clear();
    ...
    public void CheckName(string name)
    {
        ...
        if (VarIdentifiers.Contains(name)) throw new Exception($"Error parsing: {CurrentlyConsuming} varible with name \"{name}\" already exists!");
    }
    public string ParseAndResolveLabelAndInstructionAddresses(string input)
    {
        ...
        //resolve varibles
        for (int i = 0; i < VarIdentifiers.Count; i++)
        {
            // this is correct becuase count is one bigger so this works
            var byte_arr = (Declares.Count + i).ToByteArrayWithNegative(); 
            string buffer = string.Empty;
            foreach (byte bite in byte_arr)
            {
                buffer += bite.ToString("X2") + " ";
            }
            FinalText = FinalText.Replace(VarIdentifiers[i], buffer);
        }
...
#warning this may be not great
    ...
    {
        CurrentlyConsuming = "varibles";

        while (Safe)
        {
            //this is fucking terrible but whatever for now, atleast it makes it case insensitive
            //this looks for "var " at the start of a line FindStart("var ") should just have case insensitive version
            if (Current.IsEnter() && (Peek(1) is 'v' or 'V') && (Peek(2) is 'a' or 'A') && (Peek(3) is 'r' or 'R') && char.IsWhiteSpace(Peek(4)))
            {
                SkipWhitespace();
                Position += "var".Length;
                SkipWhitespace();
